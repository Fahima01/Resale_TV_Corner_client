...
const AllUsers = () => {
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://assingment-12-server-nu.vercel.app/users');
    ...
    const handleAdminRole = (id) => {
        fetch(`https://assingment-12-server-nu.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
                    ...
                    <tbody >
                        {
                            users.map((user, i) => <tr key={user._id}>
                                ...
                                <td>
                                    {user?.role !== 'admin' && <button onClick={() => handleAdminRole(user._id)} className="btn bg-green-400 btn-xs px-5 text-black hover:text-white hover:bg-orange-600">Make Admin</button>}
                                </td>
